...
const RecepcionDashboard = () => {
  ...
  // EXTRAEMOS EL CORREO DEL LOCALSTORAGE (Guardado durante el login)
  const [correo] = useState(localStorage.getItem('usuarioCorreo') || 'usuario@huesitos.com');
  ...
  return (
    // CORRECCIÓN 1: "h-screen overflow-hidden" congela la pantalla para que no crezca más allá del monitor
    <div className="h-screen overflow-hidden bg-slate-50 flex font-sans selection:bg-sky-500 selection:text-white">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200/60 shadow-sm transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 flex flex-col shrink-0`}>
        ...
        <div className="h-20 flex items-center gap-3 px-6 border-b border-slate-100 shrink-0">
          <div className="w-10 h-10 bg-gradient-to-tr from-sky-500 to-cyan-400 rounded-xl flex items-center justify-center shadow-lg shadow-sky-500/30 p-1">
            <img src={logo} alt="Logo" className="w-full h-full object-contain" />
          </div>
          <div>
            <h1 className="font-black text-slate-800 text-lg tracking-tight leading-none">Vet.Huesitos</h1>
            <p className="text-[10px] font-bold text-sky-600 uppercase tracking-widest mt-0.5">Panel Recepcionista</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 custom-scrollbar">
          <p className="px-3 text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Gestión Diaria</p>
          {menuItems.map((item) => {
            const isActive = location.pathname === item.path;
            return (
              <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold transition-all duration-200 ${isActive ? 'bg-sky-50 text-sky-600 shadow-sm border border-sky-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}>
                <div className={`${isActive ? 'text-sky-500' : 'text-slate-400'}`}>{item.icon}</div>
                {item.label}
              </Link>
            );
          })}
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200/60">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-bold text-slate-800 truncate">Recepcionista</p>
                {/* CORRECCIÓN 2: Mostramos el correo extraído de la sesión */}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 flex items-center justify-between px-4 sm:px-8 shrink-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-black text-slate-800 hidden sm:block">Panel de Control</h2>
          </div>
          
          <div className="flex items-center gap-4">
            <button className="relative p-2 text-slate-400 hover:text-sky-500 transition-colors bg-slate-50 rounded-full border border-slate-100">
              <Bell size={20} />
        ...
        {/* CORRECCIÓN 1: Solo este div puede hacer scroll (overflow-y-auto) */}
