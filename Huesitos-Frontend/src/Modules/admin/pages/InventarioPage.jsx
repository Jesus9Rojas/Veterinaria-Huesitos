...
const InventarioPage = () => {
  ...
  const [formStock, setFormStock] = useState({
    stock: '', codigoLote: '', fechaVencimiento: ''
  });
  ...
  const handleGuardarProducto = async (e) => {
    ...
    try {
      const payload = {
        ...
        fotoUrl: formProducto.fotoUrl || "/uploads/defecto-producto.png",
        activo: true
      };
  ...
  const handleDesactivarProducto = async () => {
    if (!window.confirm(`¿Estás seguro de que deseas desactivar "${formProducto.nombre}"?`)) return;
    setGuardandoProd(true);
    try {
  ...
  const handleReactivarProducto = async (id, nombre) => {
    if (!window.confirm(`¿Deseas volver a activar "${nombre}"? Volverá a aparecer en el Punto de Venta.`)) return;
    try {
      setLoading(true);
  ...
  const handleIngresarStock = async (e) => {
    ...
    try {
      const payload = {
        ...
        stock: parseInt(formStock.stock),
        codigoLote: formStock.codigoLote || "",
        fechaVencimiento: formStock.fechaVencimiento || null,
        activo: true
      };
      ...
      alert("No se pudo registrar el stock en el inventario.");
    } finally {
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-200/60">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
            <PackageSearch className="text-indigo-500" /> Gestión de Inventario
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto justify-end">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
            />
          </div>
          <button 
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200/60 overflow-hidden min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col justify-center items-center h-64 text-slate-400 bg-slate-50/50">
            ...
            <p className="text-lg font-bold text-slate-500">Catálogo vacío</p>
            <p className="text-sm">Registra tu primer producto para empezar a vender.</p>
          </div>
        ) : (
                    ...
                    <tr key={prod.id} className={`transition-colors ${inactivo ? 'bg-slate-50/80 opacity-60 grayscale' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="px-6 py-4 flex items-center gap-3">
                        <div className="w-10 h-10 bg-slate-100 rounded-lg overflow-hidden shrink-0 flex items-center justify-center border border-slate-200">
                          {prod.fotoUrl && prod.fotoUrl !== "/uploads/defecto-producto.png" ? (
                            <img src={prod.fotoUrl} alt={prod.nombre} className="w-full h-full object-cover mix-blend-multiply" />
                          ) : (
                      ...
                      <td className="px-6 py-4 text-right whitespace-nowrap w-1 space-x-2">
                        {inactivo ? (
                          ...
                          >
                            <ArchiveRestore size={16} /> Reactivar
                          </button>
                        ) : (
                          <>
                            <button 
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
              <h3 className="text-xl font-black text-slate-800 flex items-center gap-2">Detalles del Producto</h3>
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-700">{productoViendo.categoria?.nombre || 'General'}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase">Stock Actual</p>
                  <p className="text-sm font-bold text-slate-700">{productoViendo.stock || productoViendo.stockDisponible || 0} unid.</p>
                </div>
                <div className="col-span-2">
                  <p className="text-[10px] font-bold text-slate-400 uppercase">Stock Mínimo (Alerta)</p>
                  <p className="text-sm font-bold text-slate-700">{productoViendo.stockMinimo || 5} unid.</p>
                </div>
              </div>
              
              <div className="mt-4">
                <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Descripción</p>
                <p className="text-sm text-slate-600 leading-relaxed bg-slate-50 p-3 rounded-xl border border-slate-100 min-h-[60px]">
                  {productoViendo.descripcion || 'Sin descripción disponible.'}
                </p>
              </div>
            </div>
      ...
      {modalProductoOpen && createPortal(
        <div className="fixed inset-0 z-[99990] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
              <h3 className="text-xl font-black text-slate-800">{formProducto.id ? 'Editar Producto' : 'Crear Producto'}</h3>
            ...
            <form onSubmit={handleGuardarProducto} className="p-6 overflow-y-auto flex-1 space-y-4 custom-scrollbar">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">URL de la Foto (Opcional)</label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <ImageIcon className="absolute left-3 top-3.5 text-slate-400" size={18} />
                    <input type="text" value={formProducto.fotoUrl} onChange={e => setFormProducto({...formProducto, fotoUrl: e.target.value})} className="w-full pl-10 pr-3 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <select required value={formProducto.categoriaId} onChange={e => setFormProducto({...formProducto, categoriaId: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 outline-none">
                    ...
                    <button 
                      ...
                      className="bg-emerald-100 hover:bg-emerald-200 text-emerald-600 px-3 rounded-xl font-bold transition-colors shrink-0"
                    >
                      <Plus size={20} />
                    </button>
                  </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1">Precio (S/)</label>
                  <input required type="number" step="0.10" min="0" value={formProducto.precio} onChange={e => setFormProducto({...formProducto, precio: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Stock Mínimo (Alerta)</label>
                <input required type="number" min="0" value={formProducto.stockMinimo} onChange={e => setFormProducto({...formProducto, stockMinimo: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Descripción Breve</label>
                <textarea rows="2" value={formProducto.descripcion} onChange={e => setFormProducto({...formProducto, descripcion: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none"></textarea>
              </div>
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-emerald-50 shrink-0">
              <h3 className="text-lg font-black text-emerald-800 flex items-center gap-2"><Tag size={18}/> Nueva Categoría</h3>
              <button onClick={() => setModalCatOpen(false)} className="text-emerald-400 hover:text-emerald-700"><X size={20}/></button>
            </div>
            <form onSubmit={handleGuardarCategoria} className="p-6 space-y-4">
              <div>
                ...
                <input required autoFocus type="text" value={formCat.nombre} onChange={e => setFormCat({...formCat, nombre: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Descripción</label>
                <textarea rows="2" value={formCat.descripcion} onChange={e => setFormCat({...formCat, descripcion: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"></textarea>
              </div>
              <div className="flex justify-end pt-2">
                <button type="submit" disabled={guardandoCat} className="w-full px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30">
      ...
      {modalStockOpen && createPortal(
        <div className="fixed inset-0 z-[99990] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-indigo-50 shrink-0">
              <h3 className="text-xl font-black text-indigo-800 flex items-center gap-2"><PackagePlus size={20}/> Sumar Inventario</h3>
              <button onClick={() => setModalStockOpen(false)} className="text-indigo-400 hover:text-indigo-700"><X size={24}/></button>
            </div>
            <form onSubmit={handleIngresarStock} className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              <div>
                ...
                <input required type="number" min="1" value={formStock.stock} onChange={e => setFormStock({...formStock, stock: e.target.value})} className="w-full border border-indigo-200 focus:border-indigo-500 p-3 rounded-xl text-lg font-black text-center" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1">Código de Lote</label>
                  <input type="text" value={formStock.codigoLote} onChange={e => setFormStock({...formStock, codigoLote: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl uppercase" placeholder="Autogenerado" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1">Vencimiento</label>
                  <input type="date" value={formStock.fechaVencimiento} onChange={e => setFormStock({...formStock, fechaVencimiento: e.target.value})} className="w-full border border-slate-300 p-3 rounded-xl text-sm" />
                </div>
              </div>
