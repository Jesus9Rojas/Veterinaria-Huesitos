...
const FinanzasPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchTransacciones = async () => {
      try {
        ...
        if (isMounted) {
          // PROCESAMIENTO INTELIGENTE DE LOS DATOS REALES DE TU BACKEND
          const dataProcesada = res.data.map(t => {
            // 1. Extraer la fecha real de tu base de datos
            const fechaReal = t.fechaCreacion || t.fechaActualizacion;
            
            // 2. Extraer el nombre del cliente y el servicio navegando en las relaciones
            ...
            // 3. PARCHE VISUAL PARA CITAS ANTIGUAS: Si la cita vinculada está cancelada, forzar el estado de la transacción.
            let estadoReal = t.estadoPago;
            if (t.cita && t.cita.estado === 'CANCELADA') {
              estadoReal = 'RECHAZADO';
            } else if (t.pedido && t.pedido.estadoPedido === 'CANCELADO') {
              estadoReal = 'RECHAZADO';
            }
  ...
  const transaccionesFiltradas = transacciones.filter(t => {
    ...
    const matchBusqueda = busqueda 
      ? t.conceptoReal?.toLowerCase().includes(busqueda.toLowerCase()) || 
        t.estadoReal?.toLowerCase().includes(busqueda.toLowerCase()) ||
        t.medioPago?.toLowerCase().includes(busqueda.toLowerCase())
      : true;
  ...
  const totalIngresos = transaccionesFiltradas
    .filter(t => t.estadoReal === 'APROBADO')
    .reduce((sum, t) => sum + (t.monto || 0), 0);

  const totalPendientes = transaccionesFiltradas
    .filter(t => t.estadoReal === 'PENDIENTE')
    .length;
  ...
  const exportarExcel = () => {
    ...
    const rows = transaccionesFiltradas.map(t => [
      t.id,
      t.fechaReal ? new Date(t.fechaReal).toLocaleString('es-PE') : 'Sin Fecha',
      `"${t.conceptoReal || ''}"`, 
      t.medioPago || 'N/A',
      t.estadoReal,
      t.monto.toFixed(2)
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-xl border border-slate-200/60 shrink-0 w-full sm:w-auto">
          ...
          >
            <Archive size={18} /> Historial Completo
          </button>
        </div>

        <button 
          onClick={exportarExcel}
          className="bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-emerald-500/30 transition-all flex items-center justify-center gap-2 w-full sm:w-auto"
        >
        ...
        <div className={`p-6 rounded-2xl shadow-sm text-white relative overflow-hidden transition-colors duration-300 ${activeTab === 'diario' ? 'bg-gradient-to-br from-emerald-500 to-teal-400' : 'bg-gradient-to-br from-indigo-500 to-sky-400'}`}>
          <div className="relative z-10">
            <p className="text-white/80 text-sm font-bold uppercase tracking-widest mb-1">
              Ingresos {activeTab === 'diario' ? 'de Hoy' : 'Totales'}
            </p>
            <h3 className="text-3xl font-black flex items-center gap-2">
              <DollarSign size={28} /> {totalIngresos.toFixed(2)}
            </h3>
          </div>
        ...
        <div className="bg-white border border-slate-200/60 p-6 rounded-2xl shadow-sm relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-slate-400 text-sm font-bold uppercase tracking-widest mb-1">
              Pendientes de Cobro {activeTab === 'diario' ? '(Hoy)' : '(Histórico)'}
            </p>
            <h3 className="text-3xl font-black text-amber-500 flex items-center gap-2">
              <FileText size={28} /> {totalPendientes}
            </h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60">
        <div className="flex items-center gap-2 mb-4 text-slate-700 font-bold border-b border-slate-100 pb-3">
          <Filter size={18} className={activeTab === 'diario' ? 'text-emerald-500' : 'text-indigo-500'} /> Filtros de Búsqueda
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 outline-none ${activeTab === 'diario' ? 'focus:ring-emerald-500' : 'focus:ring-indigo-500'}`}
            />
          </div>

          <select value={filtroConcepto} onChange={(e) => setFiltroConcepto(e.target.value)} className={`w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 outline-none ${activeTab === 'diario' ? 'focus:ring-emerald-500' : 'focus:ring-indigo-500'}`}>
            <option value="">Todos los Conceptos</option>
            <option value="CITA">Solo Citas Médicas</option>
            <option value="TIENDA">Solo Tienda (Productos)</option>
          </select>

          <select value={filtroMedio} onChange={(e) => setFiltroMedio(e.target.value)} className={`w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 outline-none ${activeTab === 'diario' ? 'focus:ring-emerald-500' : 'focus:ring-indigo-500'}`}>
          ...
          <select value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value)} className={`w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 outline-none ${activeTab === 'diario' ? 'focus:ring-emerald-500' : 'focus:ring-indigo-500'}`}>
            <option value="">Todos los Estados</option>
            <option value="APROBADO">Aprobados</option>
            <option value="PENDIENTE">Pendientes</option>
            <option value="RECHAZADO">Rechazados / Cancelados</option>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-widest">Fecha</th>
                <th className="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-widest">Concepto</th>
                <th className="px-6 py-4 text-center text-xs font-black text-slate-500 uppercase tracking-widest">Medio Pago</th>
              ...
              {transaccionesFiltradas.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-10 text-center text-slate-400 font-medium">
                    No hay transacciones registradas {activeTab === 'diario' ? 'para el día de hoy' : ''} con los filtros aplicados.
                  </td>
                </tr>
              ) : (
                transaccionesFiltradas.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-600 whitespace-nowrap">
                      {t.fechaReal ? new Date(t.fechaReal).toLocaleString('es-PE', {
                        day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit'
                      }) : 'Sin Fecha'}
                    </td>
                    <td className="px-6 py-4 text-slate-800 font-semibold max-w-sm truncate" title={t.conceptoReal}>
                      {t.conceptoReal}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="inline-block px-2.5 py-1 bg-slate-100 text-slate-600 font-bold text-xs rounded-lg border border-slate-200">
                        {t.medioPago || 'N/A'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm ${
                        t.estadoReal === 'APROBADO' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                        t.estadoReal === 'PENDIENTE' ? 'bg-amber-100 text-amber-700 border border-amber-200' :
                        t.estadoReal === 'RECHAZADO' ? 'bg-rose-100 text-rose-700 border border-rose-200' :
                        'bg-slate-100 text-slate-700 border border-slate-200'
                      }`}>
                        {t.estadoReal}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right font-black text-slate-800 whitespace-nowrap">
                      S/. {t.monto?.toFixed(2)}
                    </td>
                  </tr>
                ))
