...
const AuxiliarPerfilPage = () => {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      const peticion = axios.post(`${import.meta.env.VITE_BACKEND_URL}api/perfiles/usuario/${usuarioId}/foto`, formData, {
        headers: {
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="relative group">
          ...
          <button 
            ...
            className="absolute bottom-0 right-0 p-2.5 bg-gradient-to-tr from-sky-500 to-cyan-400 text-white rounded-full shadow-lg border-2 border-white hover:scale-110 transition-transform disabled:opacity-50" title="Cambiar foto"
          >
            <Camera size={16} />
          </button>
        ...
        <div>
          ...
          <p className="text-sky-600 font-bold uppercase tracking-widest text-xs mt-1">{usuarioRol.replace('_', ' ')}</p>
          <p className="text-slate-500 text-sm mt-1">Gestiona tu información personal y tus credenciales de acceso al sistema.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            ...
            <div className="relative">
              ...
              <input type="text" name="nombreCompleto" value={form.nombreCompleto} onChange={handleChange} required className="w-full pl-10 border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none transition-all bg-slate-50 focus:bg-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" name="telefono" value={form.telefono} onChange={handleChange} className="w-full pl-10 border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none transition-all bg-slate-50 focus:bg-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" name="dni" value={form.dni} onChange={handleChange} className="w-full pl-10 border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none transition-all bg-slate-50 focus:bg-white" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" name="correo" value={form.correo} onChange={handleChange} required className="w-full pl-10 border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none transition-all bg-slate-50 focus:bg-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="password" name="contrasena" value={form.contrasena} onChange={handleChange} placeholder="Dejar en blanco para conservar actual" className="w-full pl-10 border border-slate-300 p-3 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none transition-all bg-slate-50 focus:bg-white" />
            </div>
          </div>
        </div>

        <div className="pt-6 border-t border-slate-100 flex justify-end">
          <button type="submit" disabled={procesando} className="bg-gradient-to-r from-sky-500 to-cyan-400 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-sky-500/30 transition-all flex items-center gap-2 disabled:opacity-50">
            {procesando ? <Activity className="animate-spin" size={18} /> : <Save size={18} />}
            {procesando ? "Guardando..." : "Guardar Perfil"}
          </button>
        </div>
      </form>
    </div>
...
export default AuxiliarPerfilPage;